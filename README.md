...
`make check-speech` makes Deepgram actually speak a word and transcribe it back, rather
than asking whether the key authenticates — the interviewer joins the room either way, so
a key that is set but out of credit used to look like an agent that never speaks. Speech
runs on one Deepgram key in both directions.
