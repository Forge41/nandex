#!/usr/bin/env bash
# Pushes the credentials in infra/.env to GitHub Actions secrets.
#
...
ENV_FILE=infra/.env
[ -f "$ENV_FILE" ] || { echo "error: $ENV_FILE is missing -- copy infra/.env.example." >&2; exit 1; }

missing=0
for name in TF_API_TOKEN RENDER_API_KEY NETLIFY_TOKEN; do
